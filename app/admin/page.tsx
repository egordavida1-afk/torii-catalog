...
export default async function AdminHome({ searchParams }: { searchParams: { q?: string; category?: string; sync?: string; updated?: string; syncError?: string; movies?: string; series?: string; anime?: string; cartoons?: string; settings?: string; kodikImported?: string; kodikUpdated?: string; kodikAttached?: string } }) {
  ...
  return (
    <>
      ...
      {searchParams.sync !== undefined && <div className="form-success">Синхронизация завершена. Kodik: новых тайтлов {Number.isFinite(kodikImported) ? kodikImported : 0}, обновлено {Number.isFinite(kodikUpdated) ? kodikUpdated : 0}, подключено к существующим {Number.isFinite(kodikAttached) ? kodikAttached : 0}. Проверено: фильмов {searchParams.movies || 0}, сериалов {searchParams.series || 0}, аниме {searchParams.anime || 0}, мультфильмов {searchParams.cartoons || 0}.</div>}
      {searchParams.syncError && <div className="form-error">Часть синхронизации завершилась с ошибкой. Открой подробности в логах и проверь доступ Kodik.</div>}
      ...
      <div className="panel auto-import-panel">
        <div className="panel-head">
          <div>
            ...
            <p className="meta">Каталог обновляется напрямую из Kodik: названия, постеры, материалы, озвучки, сезоны и серии сохраняются в нашей базе.</p>
          </div>
          <form action={syncCatalogNow}><button className="btn" type="submit" disabled={!kodikEnabled}>Обновить каталог</button></form>
        </div>
        ...
        {!kodikEnabled && <p className="source-hint">Включи автоматический поиск токена Kodik или добавь KODIK_API_TOKEN.</p>}
      </div>
